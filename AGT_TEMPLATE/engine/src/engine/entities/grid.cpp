...
engine::grid::grid(float length, float width, float step) : m_length(length), m_width(width), m_step(step)
{
	std::vector<mesh::vertex> grid_vertices;
	std::vector<uint32_t> grid_indices;
	uint32_t vertex_count = 0;
	for (float i = -m_width; i < m_width; i += m_step)
	{
	...
	for (float i = -m_length; i < m_length; i += m_step)
	{
...
engine::ref<engine::grid> engine::grid::create(float length, float width, float step)
{
